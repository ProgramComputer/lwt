...
val_p=pa.tagName.toLowerCase()+attr_p+'/'+val_p;attr_p='';pa='';node_count++});$('#adv').prepend('<p style="text-align: left;"><input style="vertical-align: middle; margin: 2px;" class="xpath" type="radio" name="xpath" value=\'/'+val_p+val1+attr_v+'\'>all: Â« /'+val_p.replace('=""','')+val1+attr_v.replace('=""','')+' Â»</input></p>');$('#adv input[type="radio"]').each(function(z){if(typeof z=='undefined')z=1;if(typeof $(this).attr('id')=='undefined'){$(this).attr('id','rb_'+z++)}
...
if(attr_p)attr_p='['+attr_p+']';val_p=$(this).parent().get(0).tagName.toLowerCase()+attr_p+'Â§'}val_p=val_p.replace('bodyÂ§','');var attrsplit=attr.substr(0,20);if(!(attrsplit==attr))attrsplit=attrsplit+'... ';if(!(attrsplit==''))attrsplit=" "+attrsplit;if(event.target==this)
...
function do_ajax_update_media_select(id){$('#mediaSelectErrorMessage').css("display","none");$('#mediaselect select').css("display","none");$('#mediaSelectLoadingImg').css("display","inherit");$.getJSON('inc/ajax.php',{action:"query",action_type:"media_paths",id:"-1"},media_select_receive_data)}
...
function make_overlib_link_change_status(txid,torder,wid,oldstat,newstat){if(oldstat==newstat){return'<span title="'+getStatusName(oldstat)+'">â</span>'}
...
function make_overlib_link_change_status_test2(wid,oldstat,newstat){let output=' <a href="set_test_status.php?wid='+wid+'&amp;status='+newstat+'&amp;ajax=1" target="ro" onclick="showRightFrames();">'+'<span title="'+getStatusName(newstat)+'">[';output+=(oldstat==newstat)?'â':getStatusAbbr(newstat);output+=']</span></a> ';return output}
...
function make_tooltip(word,trans,roman,status){const nl='\x0d';let title=word;if(roman!=''){if(title!='')title+=nl;title+='â¶ '+roman}
if(trans!=''&&trans!='*'){if(title!='')title+=nl;title+='â¶ '+trans}
if(title!='')title+=nl;title+='â¶ '+getStatusName(status)+' ['+getStatusAbbr(status)+']';return title}
...
if(getCookie(prefix+'Request]')!=""){let fetchRequest=JSON.parse(getCookie(prefix+'Request]'));deepReplace(fetchRequest,'lwt_text',text)
